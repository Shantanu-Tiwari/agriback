// import { supabase } from "../supabaseClient.js"; 
// import { v4 as uuidv4 } from "uuid"; 
// import Report from "../models/Report.js"; 
// import { Client, handle_file } from "@gradio/client";

// const uploadController = async (req, res) => { if (!req.file) return res.status(400).json({ error: "No file uploaded" });

// try { const userId = req.user.id; if (!userId) return res.status(401).json({ error: "Unauthorized" });
// } catch (err) { console.error("Controller Error:", err); return res.status(500).json({ error: err.message }); } };
// const fileName = `${uuidv4()}-${req.file.originalname.replace(/\s/g, "_")}`;

// const { data, error } = await supabase.storage
//   .from("uploads")
//   .upload(fileName, req.file.buffer, {
//     contentType: req.file.mimetype,
//   });

// if (error) throw new Error("File upload failed");

// const publicURL = `${process.env.SUPABASE_URL}/storage/v1/object/public/uploads/${fileName}`;

// const response = await fetch(publicURL);
// if (!response.ok) {
//   throw new Error(`Failed to fetch image from Supabase: ${response.status} ${response.statusText}`);
// }

// const blob = await response.blob();
// const file = new File([blob], fileName, { type: req.file.mimetype });

// const modelType = req.body.modelType || "farmer";
// const repoId =
//   modelType === "cow"
//     ? "Sid26Roy/cow_disease"
//     : modelType === "plant"
//     ? "Sid26Roy/plant_disease"
//     : "Sid26Roy/Farmer_prediction";

// const client = await Client.connect(repoId);
// const prediction = await client.predict("/predict", [handle_file(file)]);

// console.log("Gradio Response:", prediction);

// let diseaseName = "Unknown Disease";
// let confidence = "N/A";

// if (prediction?.data?.[0]) {
//   const resultLines = prediction.data[0].split("\n");
//   if (resultLines.length >= 2) {
//     diseaseName = resultLines[0].replace("Predicted: ", "").trim();
//     confidence = resultLines[1].replace("Confidence: ", "").trim();
//   }
// }

// const analysisResult = `${diseaseName} (${confidence})`;

// const newReport = new Report({
//   userId,
//   imageUrl: publicURL,
//   status: "Processed",
//   name: req.body.name || "Disease Report",
//   analysisResult,
// });

// await newReport.save();

// return res.json({
//   url: publicURL,
//   prediction: analysisResult,
//   message: `Upload successful, analyzed using ${repoId.split("/")[1]}`,
// });
// } catch (err) {
//     console.error("Controller Error:", err);
//     return res.status(500).json({ error: err.message });
//   }
// };

// export default uploadController;

// export { uploadController };export { uploadController };


import { supabase } from "../supabaseClient.js";
import { v4 as uuidv4 } from "uuid";
import Report from "../models/Report.js";
import { Client, handle_file } from "@gradio/client";

const uploadController = async (req, res) => {
    if (!req.file) return res.status(400).json({ error: "No file uploaded" });

    try {
        const userId = req.user.id;
        if (!userId) return res.status(401).json({ error: "Unauthorized" });

        const fileName = `${uuidv4()}-${req.file.originalname.replace(/\s/g, "_")}`;

        // Upload to Supabase
        const { data, error } = await supabase.storage.from("uploads").upload(fileName, req.file.buffer, {
            contentType: req.file.mimetype,
        });

        if (error) throw new Error("File upload failed");

        const publicURL = `${process.env.SUPABASE_URL}/storage/v1/object/public/uploads/${fileName}`;

        // Fetch uploaded image
        const response = await fetch(publicURL);
        if (!response.ok) {
            throw new Error(`Failed to fetch image from Supabase: ${response.status} ${response.statusText}`);
        }

        const blob = await response.blob();
        const file = new File([blob], fileName, { type: req.file.mimetype });

        // Get model type from route param
        const modelType = req.params.model;
        const repoId = modelType === "cow"
            ? "Sid26Roy/cow_disease"
            : "Sid26Roy/Farmer_prediction"; // fallback to farmer model

        // Predict with appropriate Gradio model
        const client = await Client.connect(repoId);
        const prediction = await client.predict("/predict", [handle_file(file)]);

        console.log("Gradio Response:", prediction);

        // Parse result
        let diseaseName = "Unknown Disease";
        let confidence = "N/A";

        if (prediction?.data?.length > 0) {
            const resultLines = prediction.data[0].split("\n");
            if (resultLines.length >= 2) {
                diseaseName = resultLines[0].replace("Predicted: ", "").trim();
                confidence = resultLines[1].replace("Confidence: ", "").trim();
            }
        }

        const analysisResult = `${diseaseName} (${confidence})`;

        // Save to DB
        const newReport = new Report({
            userId,
            imageUrl: publicURL,
            status: "Processed",
            name: req.body.name || "Disease Report",
            analysisResult,
        });

        await newReport.save();

        return res.json({
            url: publicURL,
            prediction: analysisResult,
            message: `Upload successful, analyzed using ${repoId.split("/")[1]}`,
        });

    } catch (err) {
        console.error("Controller Error:", err);
        return res.status(500).json({ error: err.message });
    }
};

export default uploadController;
export { uploadController };